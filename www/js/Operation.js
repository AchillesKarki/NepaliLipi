	function Operation() {
		...
		this.convert = function(word, lastLetter) {
			...
			for (var i = 0; i < container.length; i++) {
				...
				if (container[i] == word) {
					if (container[i] == "aa" || container[i] == "ii"
							|| container[i] == "uu") {
						result = result.substr(0, result.length - 1);
					} else
						this.modifyPreFormation(word);
		...
		this.modifyPostFormation = function(tempCode, word) {
			if (word[word.length - 1] in vowel
					&& word[word.length - 2] in consonant) {
				result = result.substr(0, result.length - 1);
			}
	
			if (((tempCode[tempCode.length - 1] == "i") && (word[word.length - 2] == "i"))
